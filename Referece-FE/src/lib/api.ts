...
export const fetchMatches = async (params?: { status?: string; format?: string }): Promise<Match[]> => {
  try {
    ...
    console.warn('Backend not available, using mock data for matches');
...
export const fetchMatch = async (id: number): Promise<Match> => {
  try {
    ...
    console.warn('Backend not available, using mock data for match');
...
export const fetchMatchEvents = async (id: number, after: number = 0, limit: number = 100): Promise<{ events: CommentaryEvent[], total: number, lastSequence: number }> => {
  try {
    ...
    return {
      ...
      lastSequence: MOCK_EVENTS[0].sequence
    };
  }
};
